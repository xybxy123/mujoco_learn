...
void GaitController::Reset() {
    // Only reset initialized state if parameters must be reloaded,
    // usually we don't need to rebuild actuator IDs unless model changes.
}
...
void GaitController::Initialize(const mjModel* model) {
    ...
    for (int leg_index = 0; leg_index < kLegCount; ++leg_index) {
        state_.hip_actuator_ids[leg_index] =
            mj_name2id(model, mjOBJ_ACTUATOR, kLegs[leg_index].hip_actuator);
        state_.knee_actuator_ids[leg_index] =
...
void GaitController::SetInitialPose(const mjModel* model, mjData* data) {
    ...
    for (int leg_index = 0; leg_index < kLegCount; ++leg_index) {
        JointAngles angles;
        leg_ik_.Solve(kLegs[leg_index].x_bias, kNominalHeight, angles);
        ...
        state_.filtered_commands[leg_index] = angles;
...
void GaitController::Update(const mjModel* model, mjData* data) {
    ...
    for (int leg_index = 0; leg_index < kLegCount; ++leg_index) {
        ...
        JointAngles& filtered = state_.filtered_commands[leg_index];
